  ...
  return {
    ...
    payload: {
      ...
      decisionContext: {
        ...
        decisionReady: false,
        authorityReason: "The issue appears to require a board-owned disposition, but AIA must verify authority and prepare the owner briefing first.",
        recommendation: null,
        consequences: "The issue remains in review until AIA prepares or internally reroutes this request.",
        evidenceRefs: [issueHref(issue) ?? identifier],
        risk: issue.priority === "critical" ? "critical" : issue.priority === "high" ? "high" : "medium",
        urgency: "medium",
        reversibility: "easy",
      },
