...
const routineSpecs = [
  ...
  },
  {
    ...
    description: [
      ...
      "Before preparing a protected-access decision, perform a fresh value-free check of the existing secret-ref bindings, the exact operation that succeeds or fails, and the provider's documented scoping model. If existing access already permits the bounded read, reroute the remaining technical work instead of asking for authorization again. If the provider cannot express a requested per-resource or expiry scope, do not offer that impossible grant as an owner option; record the provider boundary and use the narrowest Paperclip-enforced capability filter that does not broaden the protected action.",
      "Prepare a Polish payload.decisionContext.ownerBriefing containing: one decision sentence, a plainLanguageSummary saying what happened without internal jargon, 2-5 current facts, explicit scope/outOfScope, genuinely missing openQuestions, binding safetyConstraints, 1-5 options each with benefit/cost/risk, one explicit AIA recommendation, actions after acceptance, and rollback/recovery. State whether the request is historical or current and name adjacent functionality that it does not concern when confusion is plausible.",
