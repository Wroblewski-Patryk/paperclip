...
function companyOperatingManual() {
  return [
    ...
    "The company exists to plan, build, verify, and improve applications such as Soar and Roost.",
    ...
    "",
    "- V1 is the current local Paperclip operating target: agents finish Soar and Roost through indexed frontend/backend/worker/event-chain evidence, source-control closure, local verification, and gated deploy proof where credentials/production access are available.",
    "- Soar and Roost are both active V1 application lanes. Soar remains the first tie-breaker when two safe actions compete for the same owner, credential, or protected production gate, but Roost must not be parked when local known-state, source-control, implementation, proof, or documentation work is legal and owner-scoped.",
    "- V2.1 starts only after Roost is fully working as an app and then connects Roost/CompanyCore into Paperclip through an accepted API/MCP/data boundary.",
    "- V2.2 moves Paperclip to a dedicated VPS so agents can build applications on the server and let project pushes redeploy through Coolify to production.",
    "- V3 opens additional application projects after the local+VPS operating loop proves it can run Soar and Roost without silent idle, duplicate churn, or hidden blockers.",
    "- Aviary, Nest, Featherly, and other future apps remain parked until the board explicitly reopens them for V3 or a named exception.",
    "",
    "## Value Streams",
    "",
...
const parent = await ensureIssue(company.id, issuesByTitle, {
  ...
  description: [
    ...
    "",
    "Scope:",
    "- active first: V1 local Soar + Roost completion -> V2.1 Roost connected to Paperclip -> V2.2 Paperclip VPS builder -> V3 portfolio expansion;",
    "- future prepared: Aviary, Nest, Featherly when activated;",
...
const decisionSpecs = [
  {
    ...
    recommended: "Use the roadmap V1 local Soar + Roost completion -> V2.1 Roost connected to Paperclip -> V2.2 Paperclip VPS builder -> V3 portfolio expansion; keep Aviary, Nest, and Featherly prepared but parked until you explicitly activate them.",
    alternatives: [
...
const routineSpecs = [
  ...
  {
    title: "[Softwarehouse] Human decision inbox steward",
    owner: agent.aia ?? agent.cso ?? agent.ipm,
    goalId: companyOsGoal.id,
    priority: "high",
    cron: "45 8 * * *",
    label: "Daily human decision inbox steward at 08:45",
    description: [
      "Inspect tasks assigned to the human user.",
      "Ensure every board/user decision has plain context, one recommended option, alternatives, consequence of no answer, and work that resumes after the answer.",
      "If a decision is stale, add a short comment summarizing why it is still needed or why it can be closed.",
    ].join("\n"),
  },
  {
