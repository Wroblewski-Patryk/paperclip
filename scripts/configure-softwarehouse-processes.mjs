...
const processIssues = [
  ...
  ["[Softwarehouse][Process] Project intake and portfolio index sync", agent.portfolio, "Keep APPLICATIONS_INDEX aligned with Paperclip projects and decide which project may enter active takeover after Soar."],
...
await upsertDocument(parent.id, "softwarehouse-operating-processes", "Softwarehouse Operating Processes", [
  ...
  "| Project intake and portfolio index sync | Portfolio Director | Archived library routine |",
...
const routineSpecs = [
  ...
  {
    title: "[Softwarehouse] Project intake and index sync",
    description: "Refresh APPLICATIONS_INDEX and Paperclip project state, keep only approved active projects in motion, and name the next takeover candidate without starting it.",
    assigneeAgentId: agent.portfolio?.id ?? null,
    priority: "high",
