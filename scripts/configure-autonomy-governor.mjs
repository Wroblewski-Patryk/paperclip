...
async function main() {
  ...
  const legacyDetailedPolicyReference = [
    ...
    "7a. Keep the smallest justified next-action queue per active product: normally one worker-owned `todo` lane that Paperclip can wake. Queue depth is a ceiling and diagnostic signal, never a success target. Promote valid backlog before creating duplicates; create another child only just in time with an independent outcome, owner, evidence contract, and named dependency reason. Never replenish backlog merely because a count fell.",
    "7b. Delivery debt outranks queue depth. Before replenishing backlog or creating docs/map/status work, run the release push/deploy governor. If a clean non-divergent active-app branch is materially ahead of its deployment branch, reuse the canonical release chain and either execute the standing-consent release or resolve its first exact blocker. Do not grow the queue while an actionable source-to-production gap exists.",
    "7c. The issue portfolio has an enforced closure-only mode. At or above the configured open-issue soft limit, do not create autonomous root issues, duplicate open titles, project-truth sidecars, generic triage lanes, or broad decompositions. Reuse existing assigned delivery work, reconcile resolved blocker links, classify and commit existing repository changes, or close/cancel/consolidate stale work. Empirical application outcomes and a shrinking source-to-production gap outrank task, comment, report, map, and evidence volume.",
    "8. The owner's standing release consent permits pushing a meaningful, evidence-backed application batch to its known deployment branch when the expected effect is normal Coolify auto-redeploy; do not ask again for that qualifying push, and verify redeploy plus production smoke afterward. Manual deploy/restart/rollback, force-push, live trading or user-real-account mutation, credential changes/disclosure, destructive actions, and unknown production impact still require explicit approval in the issue/thread.",
    ...
    "13. Pending review interactions are hard stops only for genuine board/owner choices: material product scope, push/deploy, secrets or credential rotation, production/live accounts, destructive changes, money/legal/privacy, and owner acceptance. Agent-created interactions for reversible runtime choice, focused tests, docs/project-truth review, or local commit classification are misroutes: cancel them through the in-review decision-path janitor, return the issue to todo, and route the technical specialist/reviewer.",
    ...
    "22. If activeRunCount is greater than zero, do not assume the whole company is busy. Probe for one independent lane whose assignee and project are both idle; start at most one such lane, otherwise exit after read-only supervision. Do not post a comment merely to say that work is still running.",
    ...
    "32. If the control loop reports project_source_control_closure_needed, route the dirty project through its existing PM/source-control lane. Inspect diffs, preserve agent work, and decide commit/no-commit. If the resulting batch meets the standing release-consent and evidence contract, push without asking again and follow the normal auto-redeploy through production verification; otherwise record the exact remaining gate.",
    ...
    "44. Treat `start_release_delivery` and `resolve_release_delivery_blocker` from the next-legal-action selector as higher priority than blocked triage, project-truth refresh, safe non-production docs, architecture-map refresh, learning review, or generic worker backlog. A release action may be displaced only by an active incident, unsafe runtime state, dirty/divergent repository, missing exact deployment identity, or a genuine protected gate.",
    "45. A plan, report, comment, issue tree, architecture export, test inventory, or local commit is not an owner-visible milestone. Each active product must report its last deployed owner-usable milestone, current source/deployed SHA gap, exact next transition, and age of delivery debt. If no deployed milestone advances for seven days while source keeps advancing, classify this as a Softwarehouse orchestration defect and stop creating non-essential documentation work until corrected.",
    "46. Run `pnpm softwarehouse:documentation-hygiene` as the lightweight documentation guard. A missing manifest, stale/false-green truth, oversized default context, active planning/state above budget, required-architecture links into planning, or documentation-only delivery skew is a real orchestration defect. Repair the canonical source or generator; do not create another report as the fix.",
    "47. Roost is the owner-facing portfolio surface. Until its live Product Map is deployed, LUC-1910 is the default portfolio delivery priority. After deployment, every material Paperclip issue-state, release, blocker, and evidence transition must become visible through the versioned projection with freshness and conflict state; do not maintain a second manual status truth.",
    "48. Apply Goodhart/Campbell, reward-hacking, specification-gaming, Cobra-effect, Parkinson, Brooks, Conway, Gall, KISS/YAGNI, local-optimization, and Shirky checks as diagnostic lenses. Do not turn their indicators into new quotas. A valid lane changes an observable owner/product state or removes a necessary risk/dependency and carries inspectable evidence; activity counts never prove success.",
    "49. Limit automatic fan-out to one new child per product track per control cycle. More than three non-terminal direct children requires explicit scope independence, disjoint writers, and a named integration owner. After three failed attempts without materially new evidence, open one systemic-cause/escalation path instead of repeating the same action.",
    "50. Before critical, protected, cross-project, expensive, or hard-to-reverse work becomes runnable, require `executionPolicy.decisionContract`: value, urgency, cost of inaction, estimated effort, at least one resource limit, stop condition, done-enough boundary, disposition, confidence/evidence, scope, reversibility, rollback/restore/check/trigger. Low-confidence costly work is proposal/isolation/escalation, never broad autonomous execution.",
    "51. Agents have a duty not to act when work is duplicate, obsolete, hypothetical, outside active scope, low-value/high-cost, riskier than no change, or already good enough. Use later/monitor/accept_debt/reject/conditional/proposal/escalate without manufacturing replacement tasks. A heartbeat is not an activity quota.",
    "52. Active assumptions require provenance, confidence, scope, and review/expiry. Expired, invalidated, or superseded knowledge cannot seed work. Re-evaluate only traced dependants; never turn one invalid assumption into an unbounded portfolio fan-out.",
    "",
