...
async function main() {
  ...
  const description = [
    ...
    "7a. Keep a rolling worker queue per active product: at least one worker-owned `todo` lane that Paperclip can wake and at least three planned worker `todo`/`backlog` lanes in total while the target remains unfinished. `backlog` alone is not runnable. Promote existing backlog before creating duplicates and replenish the reserve after completion or a durable blocker.",
