...
const specs = [
  ...
  },
  {
    ...
    description: [
      ...
      "One productive run is not a company-wide mutex. Independent Soar, Roost, and Paperclip operating-system lanes may run concurrently, while same-agent and same-project work remains serialized.",
      "Treat `todo` as runnable and `backlog` as unselected inventory. Keep the smallest justified next-action queue per active track, normally one runnable worker todo. Queue depth is not a target; promote valid backlog before duplicates, create another child only just in time with its own outcome/evidence contract, and never replenish inventory merely because a count fell.",
