...
async function buildInstructions(definition, instructionsRoot) {
  ...
  return {
    "AGENTS.md": [
      ...
      "",
      "This is the task-scoped bundle entry file. Start from the assigned Paperclip issue and the native run context; do not preload the knowledge bundle.",
      "",
      "## Instruction Root",
      "",
      ...
      "",
      "## On-demand Knowledge Index",
      "",
      "- The files below are references, not bootstrap. Do not read them all, concatenate them, or load them before inspecting the current issue.",
      "- Retrieve only the smallest relevant excerpt when the current task cannot proceed from its task packet, evidence references, and this entry file.",
      "- Full organization history, cross-project documentation, archives, and full logs are forbidden by default. An explicit task-scoped reference does not authorize adjacent files.",
      "- Record every on-demand source used in the run evidence; prefer a stable reference over copying full content into comments or upward reports.",
      "",
      ...sharedFiles.map((file) => `- \`shared/${file}\``),
      "",
      "## On-demand Role Reference",
      "",
      `- \`${rolePath}\``,
      "- Read only when the task packet and native role responsibility leave a genuine ownership ambiguity; never use it as a reason to load every shared contract.",
      "",
      "## On-demand Metadata Reference",
      "",
      "- `metadata.md`",
      "",
      "## Tracker Boundary",
      ...
      "- If `PAPERCLIP_TASK_ID` is empty, do not load the Paperclip skill, shared contracts, role file, workspace, agent profile, or inbox. Report that no task is assigned and stop.",
      "- The safe update helper is at `Join-Path $env:LUCKYSPARROW_SOFTWAREHOUSE_ROOT 'skills/paperclip/scripts/paperclip-issue-update.mjs'`. Do not read the full Paperclip `SKILL.md` at bootstrap; retrieve only the relevant section on demand when a real assigned task requires a workflow not covered here.",
      ...
      "- Example: `$helper = Join-Path $env:LUCKYSPARROW_SOFTWAREHOUSE_ROOT 'skills/paperclip/scripts/paperclip-issue-update.mjs'`; then pipe the closeout text through stdin: `$comment | node $helper --issue-id $env:PAPERCLIP_TASK_ID --status in_review`.",
      "- Transient issue comments belong on stdin or in the operating-system temp directory. Never create `closeout*.md`, `.closeout.md`, or other transient payload files in a repository/workspace root; durable deliverables belong in their canonical tracked directory and must be attached as work products.",
      "- Organizational-memory capture is part of execution: before relying on a material unverified premise, record a deduplicated assumption with confidence/evidence/review; before closeout of an explicit owned promise, record a deduplicated commitment with its condition or due date. Use `node skills/paperclip/scripts/paperclip-organizational-memory.mjs <record|observe> --input-file <json> --dedupe-key <stable-key>`. Do not mine comments, copy routine progress or secrets, or misclassify ordinary controller outcomes as decisions.",
      ...
      "- This runtime is one bounded Windows workstation. Never spawn a fallback Paperclip/Soar/Roost/Featherly instance, broad-kill Node/PowerShell/Postgres processes, enumerate the full Win32_Process table, or overlap repo-wide validation lanes.",
      "- Canonical roots are Paperclip_Softwarehouse, Soar, Roost, and Featherly under `C:\\Personal\\Projekty\\Aplikacje`; Paperclip is fixed to port 3200 and its embedded PostgreSQL to 54329. Run `pnpm run softwarehouse:runtime-topology-audit` from the Softwarehouse root when topology is in doubt.",
      "- These four application roots are singleton checkouts. `enableIsolatedWorkspaces` must remain false and agents must not request, enable, or create isolated execution workspaces, extra clones, or Git worktrees. Use the canonical project-primary workspace and its governed local preview services; a historical isolated-workspace failure is a non-retryable policy mismatch, not an instruction to enable the feature.",
      "- Missing local tooling is not an owner decision by default. For PHP work, run `$runtime = node (Join-Path $env:LUCKYSPARROW_SOFTWAREHOUSE_ROOT 'scripts/ensure-project-runtime-capabilities.mjs') --project-root (Get-Location).Path --require-php | ConvertFrom-Json`, then invoke `& $runtime.phpExecutable ...` directly from PowerShell. Do not use the helper's nested command runner inside a Codex sandbox because nested executable spawn can be denied. For container work add `--require-docker --start-docker`; the helper performs bounded daemon readback.",
      "- Resolve reversible technical choices from evidence or route a specialist/reviewer. Do not create board interactions for local runtime choice, focused tests, docs/project-truth review, or local commit classification. Owner interactions remain mandatory for material product scope, push/deploy, secrets/credential rotation, production/live accounts, destructive changes, money/legal/privacy, and owner acceptance.",
      ...
      "- Never leave an active parent blocked by a newly created unassigned `backlog` child. A technical blocker delegated to another role must be created or immediately routed as `todo` to a competent active agent, with the smallest executable scope and an explicit return path to the parent. Use `backlog` only for intentionally deferred non-blocking work; use an owner interaction only when the missing input is genuinely owner-only.",
      "",
