...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("--run-id <id>", "Heartbeat run id for agent-authenticated mutations; falls back to $PAPERCLIP_RUN_ID")
...
export function resolveCommandContext(
  ...
  const api = new PaperclipApiClient({
    apiBase,
    apiKey,
    runId: options.runId?.trim() || process.env.PAPERCLIP_RUN_ID?.trim() || undefined,
