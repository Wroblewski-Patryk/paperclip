    ...
    if (!options.dryRun) {
      await db.transaction(async (tx) => {
        ...
        await tx.insert(activityLog).values({
          ...
          details: {
            strategy: "substantive_comment_plus_artifact",
...
export function buildBackfillCompletionEvidenceBundle(evidence: {
  ...
  const closeoutCommentRef = evidenceRef("comment", evidence.substantiveComments[0].id, "Existing closeout comment");
  const secondaryCommentRef = evidence.substantiveComments[1]
    ? evidenceRef("comment", evidence.substantiveComments[1].id, "Existing supporting comment")
    : null;
  ...
  return {
    ...
    documentationEvidence: {
      summary: "Pre-existing Paperclip-visible issue artifacts or documents preserve the deliverable and closure packet for this issue.",
