...
describe("realizeExecutionWorkspace", () => {
  it("falls back to the primary workspace when git worktree strategy starts from a non-git cwd", async () => {
    const nonGitRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-non-git-workspace-"));
    ...
    expect(workspace.cwd).toBe(nonGitRoot);
    expect(workspace.worktreePath).toBeNull();
    expect(workspace.warnings).toEqual([
      expect.stringContaining("could not be resolved as a git checkout"),
    ]);
  });
...
describe("resolveShell (shell fallback)", () => {
  ...
  const originalGitBashPath = process.env.GIT_BASH_PATH;
  ...
  afterEach(() => {
    ...
    if (originalGitBashPath !== undefined) {
      process.env.GIT_BASH_PATH = originalGitBashPath;
    } else {
  ...
  it("prefers an existing Git Bash path on Windows when SHELL is unset", () => {
    delete process.env.SHELL;
    process.env.GIT_BASH_PATH = process.execPath;
