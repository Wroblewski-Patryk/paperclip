...
describeEmbeddedPostgres("routine service live-execution coalescing", () => {
  ...
  it("skips a company-wide capacity gated routine while any company run is active", async () => {
    ...
    await db
      ...
      .set({
        ...
        description: "Run the softwarehouse governor\n\nCompany-wide capacity gate: skip while another company run is live.",
      })
      .where(eq(routines.id, routine.id));
    ...
    await expect(
      db.select().from(issues).where(eq(issues.originRunId, run.id)),
    ).resolves.toHaveLength(0);
  });
