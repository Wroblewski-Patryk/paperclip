...
describeEmbeddedPostgres("heartbeat bounded retry scheduling", () => {
  ...
  it("schedules max-turn continuation when the source context stores an issue identifier", async () => {
    ...
    expect(scheduled.outcome).toBe("scheduled");
    if (scheduled.outcome !== "scheduled") return;
    expect((scheduled.run.contextSnapshot as Record<string, unknown> | null)?.issueId).toBe(issueId);
    const wakeup = await db
      ...
      .then((rows) => rows[0] ?? null);
    expect(wakeup?.idempotencyKey).toContain(`:${issueId}:${runId}:1`);
    ...
    const refreshedIssue = await db
      ...
      .where(eq(issues.id, issueId))
      .then((rows) => rows[0] ?? null);
    expect(refreshedIssue?.executionRunId).toBe(scheduled.run.id);
  });
