...
describeEmbeddedPostgres("issue recovery actions", () => {
  ...
  it("checks out a blocked source issue with stale stranded recovery without returning 500", async () => {
    ...
    const action = await recoveryActionSvc.upsertSourceScoped({
      ...
      ownerAgentId: managerId,
      cause: "stranded_assigned_issue",
      fingerprint: "stranded:checkout-recovery",
      evidence: { latestIssueStatus: "blocked" },
      nextAction: "Restore a live execution path.",
      wakePolicy: { type: "wake_owner" },
    });
    ...
    const checkedOut = await request(app)
      .post(`/api/issues/${sourceIssueId}/checkout`)
      .send({ agentId: coderId, expectedStatuses: ["blocked"] })
      .expect(200);
    ...
    const activityRows = await db
      ...
      .where(eq(activityLog.entityId, sourceIssueId));
    expect(activityRows.map((row) => row.action)).toEqual(
      expect.arrayContaining(["issue.checked_out", "issue.recovery_action_resolved"]),
    );
    expect(activityRows.find((row) => row.action === "issue.recovery_action_resolved")?.details).toMatchObject({
      source: "source_revalidation",
      trigger: "checkout",
