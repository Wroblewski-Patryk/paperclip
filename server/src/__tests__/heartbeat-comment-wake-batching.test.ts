...
describe("heartbeat comment wake batching", () => {
  ...
  it("does not reopen a done issue for deferred gate-watcher no-op closure comments", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const assigneeAgentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "Keep done gate-watcher closure done",
        status: "todo",
        priority: "medium",
        assigneeAgentId,
        issueNumber: 1,
      ...
      const comment = await db
        ...
        .values({
          ...
          authorAgentId: assigneeAgentId,
          createdByRunId: firstRun?.id ?? null,
          body: "Gate-watcher no-op closure posted after terminal disposition; no further action required.",
        })
        .returning()
        .then((rows) => rows[0]);
      ...
      await waitFor(async () => {
        const cancelledDeferred = await db
          ...
          .then((rows) => rows[0] ?? null);
        return Boolean(cancelledDeferred);
      }, 90_000);
      await waitFor(async () => {
        const runs = await db
          .select()
