...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Any transition to `done` must include a typed `completionEvidence` bundle with test, review, documentation, and `learningDisposition` evidence; high-risk work also needs security, deployment, and monitoring evidence.",
  "- Classify learning as `not_applicable`, `one_off`, or `systemic`. A systemic problem must include evidence that prevention was implemented or reference a separate same-company prevention issue.",
  ...
  "- Every possible board-facing interaction must include payload.decisionContext. Non-AIA agents set decisionReady:false and must not impersonate AIA; the AIA decision steward classifies it before the owner sees it. AIA may mark it ready only after adding ownerBriefing in Polish with a plain-language summary, 2-5 facts, explicit scope and exclusions, missing information, safety constraints, compared options (benefit/cost/risk), one recommendation, post-decision actions, and rollback. State whether the matter is historical or current and what adjacent functionality it does not concern when confusion is plausible. Before escalating protected access, freshly verify existing secret-ref bindings, the exact operation currently authorized, and the provider's real scoping model; do not ask the board to choose an unsupported per-resource grant or to re-authorize access already sufficient for the bounded read. Route technical review to the responsible agent instead of the board; never ask the board to paste a secret value—request a secret-ref alias only.",
...
export function buildPaperclipEnv(agent: {
  ...
  if (
    ...
    const instructionsRootPath = (agent.adapterConfig as Record<string, unknown>).instructionsRootPath;
    if (typeof instructionsRootPath === "string" && instructionsRootPath.trim().length > 0) {
      vars.PAPERCLIP_AGENT_INSTRUCTIONS_ROOT = instructionsRootPath.trim();
    }
  }
