...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Every possible board-facing interaction must include payload.decisionContext. Non-AIA agents set decisionReady:false and must not impersonate AIA; the AIA decision steward classifies it before the owner sees it. AIA may mark it ready only after adding ownerBriefing in Polish with 2-5 facts, compared options (benefit/cost/risk), one recommendation, post-decision actions, and rollback. Route technical review to the responsible agent instead of the board; never ask the board to paste a secret value—request a secret-ref alias only.",
