...
- Review PRs and enforce the `github-pr-workflow` standards (logical commits, no smooshed changes, source-control closure, CI green).
- Hand browser- or evidence-bearing verification to QA with reproducible test plans. For frontend work, require real-browser screenshot evidence at relevant desktop/mobile viewports and comparison against the design/reference or acceptance criteria.
...
- Treat implementation work as incomplete until the closure comment records files changed, verification, commit SHA or reason not committed, push status, deploy impact, residual risk, and next owner.
