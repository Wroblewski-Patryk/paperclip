...
function buildWakeText(
  ...
  const lines = [
    ...
    "   - Every possible board-facing interaction must include payload.decisionContext. Non-AIA agents set decisionReady:false and do not impersonate AIA. AIA marks it ready only after adding a Polish ownerBriefing with a plain-language summary, 2-5 facts, explicit scope and exclusions, missing information, safety constraints, compared options (benefit/cost/risk), a recommendation, post-decision actions, and rollback. It states whether the matter is historical or current and what adjacent functionality it does not concern when confusion is plausible. Technical review belongs with the responsible agent, and protected input must reference a secret alias rather than contain a secret value.",
