...
import {
  assertSufficientBackupDiskSpace,
...
describe("assertSufficientBackupDiskSpace", () => {
  it("fails fast when the backup volume has less free space than the configured floor", () => {
    const tempDir = createTempDir("paperclip-backup-disk-guard-");
    ...
    ).toThrow(/Refusing database backup: backup volume has/);
  });

  it("applies retention pruning before disk-space preflight", async () => {
    const backupDir = createTempDir("paperclip-backup-preflight-prune-");
    const filenamePrefix = "paperclip-preflight";
    const staleBackupFile = path.join(backupDir, `${filenamePrefix}-stale.sql.gz`);
    const emptyBackupFile = path.join(backupDir, `${filenamePrefix}-empty.sql.gz`);
    fs.writeFileSync(staleBackupFile, "stale");
    fs.closeSync(fs.openSync(emptyBackupFile, "w"));
    const staleMtime = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
    fs.utimesSync(staleBackupFile, staleMtime, staleMtime);
    fs.utimesSync(emptyBackupFile, staleMtime, staleMtime);
    ...
    ).rejects.toThrow(/Refusing database backup/);

    expect(fs.existsSync(staleBackupFile)).toBe(false);
    expect(fs.existsSync(emptyBackupFile)).toBe(false);
  });

  it("caps retained backup bytes after age and count retention while preserving the newest backup", async () => {
    const backupDir = createTempDir("paperclip-backup-size-cap-");
    const filenamePrefix = "paperclip-size-cap";
    const now = Date.now();
    const backups = [
      { name: "oldest", size: 200 },
      { name: "middle", size: 200 },
      { name: "newest", size: 200 },
    ].map((backup, index) => {
      const backupFile = path.join(backupDir, `${filenamePrefix}-${backup.name}.sql.gz`);
      fs.writeFileSync(backupFile, "x".repeat(backup.size));
      const mtime = new Date(now - (3 - index) * 60_000);
      fs.utimesSync(backupFile, mtime, mtime);
      return backupFile;
    });
    ...
    await expect(
      runDatabaseBackup({
        ...
        filenamePrefix,
        diskSpaceGuard: false,
      }),
    ).rejects.toThrow();

    expect(fs.existsSync(backups[0])).toBe(false);
    expect(fs.existsSync(backups[1])).toBe(true);
    expect(fs.existsSync(backups[2])).toBe(true);
  });
});

describe("runDatabaseBackup lock guard", () => {
  it("fails fast when another backup owns the same backup directory", async () => {
    const backupDir = createTempDir("paperclip-db-backup-lock-");
    const filenamePrefix = "paperclip-lock-test";
    const lockFile = path.join(backupDir, `.${filenamePrefix}.backup.lock`);
    fs.writeFileSync(lockFile, JSON.stringify({ pid: 123, createdAt: new Date().toISOString() }));
    ...
    ).rejects.toThrow(/Database backup already in progress/);

    expect(fs.existsSync(lockFile)).toBe(true);
    expect(
      fs.readdirSync(backupDir).filter((name) => name.endsWith(".sql") || name.endsWith(".sql.gz")),
    ).toEqual([]);
  });
});
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    "prunes empty backups and caps same-day daily retention",
    async () => {
      const sourceConnectionString = await createTempDatabase();
      const backupDir = createTempDir("paperclip-db-backup-prune-");
      const sql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const filenamePrefix = "paperclip-prune-test";

      try {
        await sql.unsafe(`
          CREATE TABLE "public"."backup_prune_records" (
            "id" serial PRIMARY KEY,
            "note" text NOT NULL
          );
          INSERT INTO "public"."backup_prune_records" ("note")
          VALUES ('keep me');
        `);

        const now = Date.now();
        const oldBackupFiles: string[] = [];
        for (let index = 0; index < 7; index += 1) {
          const backupFile = path.join(backupDir, `${filenamePrefix}-same-day-${index}.sql.gz`);
          fs.writeFileSync(backupFile, `backup ${index}`);
          const mtime = new Date(now - (index + 1) * 60_000);
          fs.utimesSync(backupFile, mtime, mtime);
          oldBackupFiles.push(backupFile);
        }

        const emptyBackupFile = path.join(backupDir, `${filenamePrefix}-empty.sql.gz`);
        fs.closeSync(fs.openSync(emptyBackupFile, "w"));
        const emptyMtime = new Date(now - 30_000);
        fs.utimesSync(emptyBackupFile, emptyMtime, emptyMtime);
        ...
        expect(fs.existsSync(emptyBackupFile)).toBe(false);
        expect(oldBackupFiles.filter((backupFile) => fs.existsSync(backupFile))).toHaveLength(5);
      } finally {
        await sql.end();
      }
    },
