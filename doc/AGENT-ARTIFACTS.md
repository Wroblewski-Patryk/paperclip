...
```sh
node skills/paperclip/scripts/paperclip-upload-artifact.mjs path/to/output.webm \
  --title "Walkthrough render" \
  --summary "Rendered walkthrough for review"
```

The Bash helper remains available for POSIX environments:
...
2. Upload it with `node skills/paperclip/scripts/paperclip-upload-artifact.mjs`.
...
```sh
node skills/paperclip/scripts/paperclip-upload-artifact.mjs render.bin \
  --title "Demo video render" \
...
On Windows, do not synthesize ad-hoc `powershell.exe -Command ... curl.exe ...`
chains for attachment uploads or issue updates. Use the Node helpers instead.

If the helper is unavailable, prefer a small Node `fetch`/`FormData` script.
Use raw `curl` only as a last resort on trusted systems:
