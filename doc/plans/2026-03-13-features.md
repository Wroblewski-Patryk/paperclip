...
- V1 budget incident lifecycle is explicit and auditable with required states:
  `detected -> warned_80 -> paused_100 -> override_requested -> override_decided -> resumed_or_closed`,
  and each transition records actor, timestamp, prior/new state, and reason in the issue/activity trail.
- V1 board override path is fail-closed and bounded:
  execution remains blocked until a board `request_confirmation` interaction is explicitly accepted,
  rejected/expired/invalid responses keep the issue paused, and accepted overrides must include scope
  (`single_issue` or `time_boxed_window`) plus an expiry checkpoint that auto-returns to paused when reached.
