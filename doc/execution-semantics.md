...
## 7. Architecture Drift And Evidence Register Materialization

Architecture awareness exports, proof registers, gap registers, function-journey indices, and similar drift reports are evidence surfaces. They are not, by themselves, an executable disposition.

When Paperclip uses one of these surfaces to supervise a product or repository, every actionable row must eventually resolve to one of these durable outcomes:

- a linked issue that owns the implementation, audit, proof collection, or decision
- a linked blocker or approval that names the external input required before work can continue
- a linked evidence artifact proving the gap is already closed
- an explicit defer/duplicate decision with owner, reason, and revisit condition
- a correction to the register when the row is stale or incorrect

A refresh-only task may update the register, but it must not be treated as closure for the drift itself unless the refreshed register records one of those outcomes for each actionable row. "Indexed", "refreshed", "read", or "known" are therefore intermediate states, not completion states, for architecture-to-code drift.

### Materialization fingerprint

Backlog materialization must be duplicate-safe. The canonical fingerprint is:

- `(projectId, sourceRegisterPathOrId, sourceRowId)`

If the same register row is seen again, Paperclip must reuse the existing issue, blocker, evidence link, or defer decision instead of creating a new sibling task. A later register revision may update the linked issue when the source row changed materially, but it must preserve the trace to the prior disposition.

### Work sizing

Materialization should prefer one-owner issues with a narrow proof contract. It may create parent/child trees for large drift classes, but the parent is only healthy when each child has an owner, a live path, or a blocker. For very large registers, Paperclip may process a bounded batch per control tick, as long as the unprocessed remainder stays visible as pending materialization work.

### Exit condition

A product area is architecture-aligned only when its current drift/evidence register has no actionable row without a durable outcome. Local-only proof may be a valid outcome for local readiness, but it must remain a visible proof gap when the product contract requires browser, authenticated, production, VPS, exchange-adapter, or protected-gate evidence.

## 8. Accepted-Plan Decomposition
...
## 9. Non-Terminal Issue Liveness Contract
