...
**Step 6 — Understand context.** Prefer `GET /api/issues/{issueId}/heartbeat-context` first. It gives you compact issue state, ancestor summaries, goal/project info, comment cursor metadata, and a bounded `companyOrientation` packet drawn from current goals, organizational memory, learning, capacity, and attention signals without forcing a full thread replay. Use `?includeCompanySituation=full` only when the bounded orientation is insufficient; use `none` only for a deliberately minimal diagnostic read.
