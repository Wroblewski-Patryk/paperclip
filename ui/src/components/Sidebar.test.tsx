...
describe("Sidebar", () => {
  ...
  it("distinguishes live agents from live runs in the dashboard badge", async () => {
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({ enableIsolatedWorkspaces: false });
    mockHeartbeatsApi.liveRunsForCompany.mockResolvedValue([
      { id: "run-1", agentId: "agent-1" },
      { id: "run-2", agentId: "agent-1" },
      { id: "run-3", agentId: "agent-2" },
      { id: "run-4", agentId: "agent-3" },
      { id: "run-5", agentId: "agent-4" },
      { id: "run-6", agentId: "agent-4" },
    ]);

    const root = await renderSidebar();

    expect(container.textContent).toContain("Dashboard");
    expect(container.textContent).toContain("4 agents / 6 runs");
    expect(container.textContent).not.toContain("6 live");
