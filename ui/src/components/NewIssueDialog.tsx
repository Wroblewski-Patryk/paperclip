...
import type { IssueWorkMode, SoftwarehouseIssueTemplate } from "@paperclipai/shared";
...
function templateDocumentKey(template: SoftwarehouseIssueTemplate) {
  return template.defaultDocumentKey ?? slugifyDocumentKey(template.kind);
}

function templateFileName(template: SoftwarehouseIssueTemplate, documentKey: string) {
  return `${documentKey}-${template.key || template.kind}.md`;
}
...
export function NewIssueDialog() {
  ...
  const {
    data: issueTemplateCatalog,
    isError: issueTemplateCatalogError,
    isLoading: issueTemplateCatalogLoading,
  } = useQuery({
    queryKey: queryKeys.softwarehouse.issueTemplates(effectiveCompanyId!),
    queryFn: () => softwarehouseApi.issueTemplates(effectiveCompanyId!),
    enabled: !!effectiveCompanyId && newIssueOpen,
    retry: false,
  });
  ...
  const applyTemplateToDescription = useCallback((template: SoftwarehouseIssueTemplate, mode: "append" | "replace") => {
    const currentDescription = descriptionRef.current.trim();
    const nextDescription = mode === "append" && currentDescription
      ? `${currentDescription}\n\n${template.body.trim()}`
      : template.body.trim();
    setIssueText(titleRef.current, nextDescription);
    queueDraftSave({ description: nextDescription });
    setTemplateOpen(false);
  }, [queueDraftSave, setIssueText]);

  const stageTemplateDocument = useCallback((template: SoftwarehouseIssueTemplate) => {
    setStagedFiles((current) => {
      const baseDocumentKey = templateDocumentKey(template);
      const documentKey = createUniqueDocumentKey(baseDocumentKey, current);
      const file = new File([template.body], templateFileName(template, documentKey), {
        type: "text/markdown",
      ...
      return [
        ...current,
        {
          id: `template:${template.key}:${documentKey}:${file.lastModified}`,
          file,
          kind: "document",
          documentKey,
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          >
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <Popover open={templateOpen} onOpenChange={setTemplateOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    Templates
                    {issueTemplateCatalogLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : null}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[min(30rem,calc(100vw-2rem))] p-2" align="start">
                  {issueTemplateCatalogError ? (
                    <div className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive">
                      Template catalog could not be loaded. Manual issue creation is still available.
                    </div>
                  ) : issueTemplateCatalogLoading ? (
                    ...
                    <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
                      {(issueTemplateCatalog?.templates ?? []).map((template) => {
                        const hasDescription = descriptionRef.current.trim().length > 0;
                        const documentKey = templateDocumentKey(template);
                        return (
                          <div key={template.kind} className="rounded-md border border-border/70 p-2">
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="text-sm font-medium">{template.label}</div>
                                <div className="mt-0.5 text-xs text-muted-foreground">{template.description}</div>
                                <div className="mt-1 text-[11px] text-muted-foreground">{template.useCase}</div>
                              </div>
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-1.5">
                              {hasDescription ? (
                                <>
                                  <Button
                                    ...
                                    onClick={() => applyTemplateToDescription(template, "append")}
                                  >
                                    Append
                                  </Button>
                                  <Button
                                    ...
                                    size="sm"
                                    className="h-7 px-2 text-xs"
                                    onClick={() => applyTemplateToDescription(template, "replace")}
                                  >
                                    Replace
                                  </Button>
                                </>
                              ...
                              <Button
                                ...
                                onClick={() => stageTemplateDocument(template)}
                              >
              ...
              {issueTemplateCatalogError ? (
                <span className="text-xs text-destructive">Template fetch failed</span>
              ) : null}
            </div>
            <div
              className={cn(
