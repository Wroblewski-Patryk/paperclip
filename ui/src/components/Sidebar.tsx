...
export function Sidebar() {
  ...
  const liveAgentCount = liveRuns ? new Set(liveRuns.map((run) => run.agentId)).size : 0;
  const liveLabel = liveRunCount > 0 && liveAgentCount > 0 && liveAgentCount !== liveRunCount
    ? `${liveAgentCount} agents / ${liveRunCount} runs`
    : undefined;
  const liveTitle = liveRunCount > 0
    ? liveAgentCount === liveRunCount
      ? `${liveRunCount} live ${liveRunCount === 1 ? "run" : "runs"} across ${liveAgentCount} ${liveAgentCount === 1 ? "agent" : "agents"}`
      : `${liveRunCount} live runs across ${liveAgentCount} live agents`
    : undefined;
    ...
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      ...
      <div className="flex items-center gap-1 px-3 h-12 shrink-0">
        ...
        <SidebarSection label="Company">
          <SidebarNavItem to="/softwarehouse" label="Softwarehouse" icon={BrainCircuit} />
