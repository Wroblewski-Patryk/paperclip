...
export function ApprovalCard({
  ...
  const localizedKindLabel = language === "pl"
    ? {
        hire_agent: "Zatrudnienie agenta",
        approve_ceo_strategy: "Strategia firmy",
        budget_override_required: "Przekroczenie budżetu",
        request_board_approval: "Formalna zgoda",
      }[approval.type] ?? kindLabel
    : kindLabel;
  const localizedStatus = language === "pl"
    ? {
        pending: "Oczekuje",
        revision_requested: "Wymaga poprawy",
        approved: "Zatwierdzono",
        rejected: "Odrzucono",
        cancelled: "Anulowano",
      }[approval.status] ?? approval.status.replace(/_/g, " ")
    ...
    <div className="paperclip-surface p-4">
      <div className="flex items-start justify-between gap-4">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {requesterAgent && (
                  <div className="inline-flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
                    <span>{language === "pl" ? "Wniosek od" : "Requested by"}</span>
              ...
              <div className="space-y-1">
                <h3 className="text-base font-semibold leading-6 text-foreground">
                  {subject ?? localizedKindLabel}
                </h3>
                <p className="text-xs leading-5 text-muted-foreground">
                  {language === "pl"
                    ? `Utworzono ${formatDateTime(approval.createdAt)}`
        ...
        <div className="shrink-0">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-border/70 bg-background/80 px-2.5 py-1 text-xs text-muted-foreground">
            ...
            <span className="capitalize">{localizedStatus}</span>
          </div>
        </div>
      </div>
      ...
      {approval.decisionNote && (
        <div className="mt-4 rounded-lg border border-border/60 bg-muted/30 px-3.5 py-3 text-xs leading-5 text-muted-foreground">
          <span className="font-medium text-foreground">{language === "pl" ? "Uzasadnienie decyzji." : "Decision note."}</span> {approval.decisionNote}
        </div>
      )}
              ...
              <>
                ...
                >
                  {pendingAction === "approve"
                    ? language === "pl" ? "Zatwierdzanie…" : "Approving..."
                    : language === "pl" ? "Zatwierdź" : "Approve"}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                ...
                >
                  {pendingAction === "reject"
                    ? language === "pl" ? "Odrzucanie…" : "Rejecting..."
