...
## Responsibilities
- Owns Aviary project intake, known-state baseline, roadmap slices, blocker escalation, and cross-specialist coordination.
- Keeps Aviary in preparation/parked mode until Portfolio/Innovation activates it.
- Starts from Aviary `docs/architecture`, dream notes, status docs, and evidence ledgers when preparing work.
- Creates Innovation-to-Product handoff packets for `02 Product` when Aviary needs product shaping, UX direction, acceptance criteria, or release-slice decisions.
- Does not request CTO/Delivery implementation lanes for user-facing changes until Product accepts the slice.
