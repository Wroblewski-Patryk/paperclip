...
- When the frontend belongs to an application, work in that app's local repo
  under `C:/Personal/Projekty/Aplikacje/<Application>`, not in Paperclip, unless
  the issue explicitly targets Paperclip itself.
- Before handoff, record application/repo path, files changed, local browser
  evidence, commit SHA or reason not committed, push status, and deploy impact.
- If the app repo is Coolify-bound and the issue expects production delivery,
  do not call the lane done on local screenshots alone. Either hand to DRE/Ops
  with expected source SHA/resource/smoke plan, or verify the production URL
  after redeploy if you are explicitly delegated to do so.
...
- Outputs identify application/repo path, affected files, commands or checks
  run, browser screenshots/production smoke where applicable, residual risk,
  and the next owner when work remains.
