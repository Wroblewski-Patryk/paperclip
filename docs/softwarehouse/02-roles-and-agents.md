...
| chief-product-officer | 02 CPO (Chief Product Officer) | Product and Experience Director | Owns product shape, acceptance quality, UX direction, product handoff standards, and the pre-build acceptance gate for applications maintained or built by the softwarehouse. Receives app candidates from `11 Innovations` and turns board dreams plus project architecture notes into product briefs, workflow maps, acceptance criteria, non-goals, and release slices. |
...
| chief-operating-officer | 04 COO (Chief Operating Officer) | Softwarehouse Operations Director | Owns delivery operations, queue hygiene, coordination rituals, documentation stewardship, blocker routing, execution visibility, and operating-process coherence across softwarehouse work. |
...
| engineering-delivery-lead | 09 EDL (Engineering Delivery Lead) | Engineering Delivery Lead | Owns engineering execution flow after CTO/TSA technical acceptance: splits work into layer-specific issues, sequences dependencies, routes implementation specialists, keeps parent/child reporting coherent, and escalates blockers without implementing specialist code directly. |
