# LUC-827 Evidence Gate and Definition of Done Review
...
- whether recent Soar/Roost issue records still expose inspectable evidence;
...
- `server/src/routes/issues.ts` still uses `hasIssueDoneEvidence()` as the
  non-comment evidence check.
- `hasIssueDoneEvidence()` still accepts any existing issue document,
  attachment, or work product.
- `docs/agent-policy-gates.md` and `docs/agent-evidence.md` still describe the
  split honestly: the API hard gate requires inspectable evidence existence,
  while typed `TEST`/`REVIEW`/`DOCS` and applicable
  `SECURITY`/`DEPLOY`/`MONITORING` remain an operating requirement rather than
  a product-enforced bundle.

Working tree readback:

- `git status --short`
  - result: clean before this report was written

## Live Readback

- [LUC-821](/LUC/issues/LUC-821) is `done` as of
  `2026-07-12T21:13:08.749Z` and still exposes inspectable closure evidence:
  `1` attachment and `5` comments.
- [LUC-822](/LUC/issues/LUC-822) is `done` as of
  `2026-07-12T21:10:34.027Z` and still exposes inspectable closure evidence:
  `1` attachment and `1` comment.
- [LUC-25](/LUC/issues/LUC-25) is still `blocked` as of
  `2026-07-12T00:18:26.390Z`, with `3` first-class blockers and `1`
  attachment. The parent remains fail-closed instead of overstating readiness.
- [LUC-824](/LUC/issues/LUC-824) already documented the current product gap:
...
behind first-class blockers. That is the correct current health signal for this
routine.
