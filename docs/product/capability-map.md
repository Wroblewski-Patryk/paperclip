...
| SH-CAP-019 | Cost and quota governance | Operators can distinguish billed API spend from effective subscription usage, while the scheduler preserves enough quota to finish and verify useful work. | Cost ledger, subscription-equivalent plan share, provider quota windows, per-agent budget policies, hard-stop enforcement, and the Costs control surface are wired into heartbeat admission. | implemented | `server/src/services/costs.ts`, `server/src/services/budgets.ts`, `server/src/services/heartbeat.ts`, `ui/src/pages/Costs.tsx`, `pnpm softwarehouse:extension-utilization` | Keep billed spend and plan quota visibly separate; calibrate scheduler thresholds from live quota evidence rather than disabling the hold. |
