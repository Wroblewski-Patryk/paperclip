...
- Treat test workspaces and evidence files under the operating-system temporary directory as disposable, owned resources. After tests or interrupted runs, use `pnpm run softwarehouse:temp-hygiene` to audit them and `pnpm run softwarehouse:temp-hygiene:apply` to remove stale items. The cleanup must reject live-process references, validate every target under the exact temporary root, unlink junctions without traversing them, and preserve canonical application roots.
...
- Keep disposable logs, API snapshots, closeout request bodies, browser captures, and completion-evidence staging files out of the repository root. Use the operating-system temporary directory or an authoritative artifact/work-product location, then remove the disposable copy. `softwarehouse:temp-hygiene` audits both owned OS-temp resources and narrowly named, untracked root leftovers; `softwarehouse:temp-hygiene:apply` removes only validated stale candidates.
